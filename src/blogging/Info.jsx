...
const Info = () => {
  ...
  return (
    <>
      ...
      {/* Header Section: Responsive Typography and Padding */}
      ...
      {/* Blog Content: Responsive Spacing and Text Sizes */}
      ...
      <div className="border-t border-b py-8 md:py-12 px-4 sm:px-6 bg-gray-50">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8 sm:gap-4">
          {nav.prev ? (
            <Link to={`/blog/${nav.prev.id}`} className="group w-full sm:max-w-[45%]">
              <p className="text-[10px] md:text-xs text-gray-400 uppercase mb-1 tracking-widest">← Previous</p>
          ...
          {nav.next ? (
            <Link to={`/blog/${nav.next.id}`} className="group w-full sm:max-w-[45%] text-left sm:text-right">
              <p className="text-[10px] md:text-xs text-gray-400 uppercase mb-1 tracking-widest">Next →</p>
              <h4 className="font-bold text-base md:text-lg group-hover:text-blue-600 transition-colors line-clamp-2 sm:line-clamp-1">
                {nav.next.title}
              </h4>
            </Link>
